    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-black to-gray-900 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="glass-card rounded-2xl p-6 border border-purple-500/20">
          ...
          <button onClick={() => handleGoogleClick('user')} className="w-full glass-card-hover border border-purple-500/20 text-white py-3 rounded-lg font-bold mb-4 transition">
          ...
          <button onClick={() => handleGoogleClick('admin')} className="w-full glass-card-hover py-2.5 rounded-lg text-xs text-gray-300 font-bold border border-purple-500/20">⚙️ Khusus Admin (Google)</button>
        </div>

        {showGooglePicker && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
            <div className="glass-card rounded-2xl p-6 border border-purple-500/20 w-full max-w-sm">
              <p className="text-white font-bold mb-4">Pilih Akun Google</p>
              <button onClick={() => selectGoogleAccount('musisi@gmail.com')} className="w-full glass-card-hover border border-purple-500/20 text-white py-3 rounded-lg mb-2 text-sm">
                👤 musisi@gmail.com
              </button>
              <button onClick={() => selectGoogleAccount('other@gmail.com')} className="w-full glass-card-hover border border-purple-500/20 text-white py-3 rounded-lg mb-4 text-sm">
                👤 other@gmail.com
              </button>
              <button onClick={() => setShowGooglePicker(false)} className="w-full bg-red-700 text-white py-2 rounded-lg text-sm font-bold">Batal</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 scrollbar-hide">
          {activeChat.messages.map(msg => (
            <div key={msg.id} className={`flex ${msg.from === 'me' ? 'justify-end' : 'justify-start'}`}>
              <div className={`px-4 py-2 rounded-2xl max-w-xs ${msg.from === 'me' ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white' : 'glass-card text-gray-300 border border-purple-500/20'}`}>
...
function AdminDashboard({ addToast, adminMessages, setAdminMessages }) {
  ...
  const sendChatMessage = () => {
    ...
    setChatMessages(prev => ({
      ...prev,
      [selectedChat]: [...(prev[selectedChat] || []), { id: Date.now(), from: 'admin', text: chatMessage }]
    }));
    setChatMessage('');
    addToast('💬 Pesan terkirim!');
  };
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 scrollbar-hide">
          {(chatMessages[selectedChat] || []).map(msg => (
            <div key={msg.id} className="flex justify-start">
              <div className="glass-card text-gray-300 border border-purple-500/20 px-4 py-2 rounded-2xl max-w-xs">
                <p className="text-sm">{msg.text}</p>
              </div>
            </div>
          ))}
