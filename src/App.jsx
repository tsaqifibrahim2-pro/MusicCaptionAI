...
function LoginPage({ onDemoUser, onDemoAdmin }) {
  ...
  const adminGoogleAccounts = [
    { id: 1, email: 'musisi@gmail.com', name: 'Admin Musisi' },
  ];
  ...
  const selectAdminAccount = (account) => {
        ...
        {showGooglePicker && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
            <div className="glass-card rounded-2xl p-6 border border-purple-500/20 w-full max-w-sm">
              <p className="text-white font-bold mb-4">Pilih Akun Google Admin</p>
              <div className="space-y-2 mb-4">
                {adminGoogleAccounts.map(account => (
                  <button 
                    key={account.id} 
                    onClick={() => selectAdminAccount(account)} 
                    className="w-full glass-card-hover border border-purple-500/20 text-white py-3 rounded-lg text-sm text-left px-3 flex items-center gap-3"
                  >
      ...
      <div className="flex flex-col h-screen bg-black">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 scrollbar-hide">
          {(activeChat.messages || []).map(msg => (
            <div key={msg.id} className={`flex ${msg.from === 'me' || msg.from === 'Admin' ? 'justify-end' : 'justify-start'}`}>
              <div className={`px-4 py-2 rounded-2xl max-w-xs ${msg.from === 'me' || msg.from === 'Admin' ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white' : 'glass-card text-gray-300 border border-purple-500/20'}`}>
  ...
  if (selectedChat) {
    return (
      <div className="flex flex-col h-screen bg-black">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 scrollbar-hide">
          {adminMessages.filter(m => m.from === selectedChat || m.from === 'User' || m.from === 'Admin').map(msg => (
